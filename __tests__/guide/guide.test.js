...
describe("гостевой эндпоинт", () => {
  ...
  it("гость остаётся гостем, даже если прислал роль врача", async () => {
    await request(appWithGuest())
      .post("/api/v1/public/guide/ask")
      .send({ role: "doctor", messages: [{ role: "user", content: "Привет" }] });

    // Роль в гостевом входе не читается вообще: подставить себе врача,
    // прислав слово в теле запроса, нельзя.
