...
/**
 * Предусловия, которые проверяются ДО обращения к модели.
 *
 * Каждый круг цикла — два вызова Opus с рассуждением, и тратить их на кейс,
 * который всё равно нельзя опубликовать по структурным причинам, бессмысленно:
 * агент вернёт тот же список блокеров, но бесплатно и мгновенно.
 */
function prerequisites(doc) {
  const blockers = [];

  if (!doc.images?.length) {
    blockers.push("загрузите снимок — агент запускается после кадра");
  }

  // План есть, а на кадре ничего не размечено: находки из плана не перенесены
  // на холст. Такой кейс нельзя публиковать — учащемуся нечего находить, а
  // расставить точки может только человек, который снимок видел.
  //
  // Пустой план при пустой разметке блокером НЕ считается: это кейс «норма»,
  // где находок нет по замыслу автора.
...
export async function runRadiologyCaseAgent({
  ...
  const blockers = collectPublishBlockers(fresh);
