...
// намеренно: черновик агент берёт из базы, а не из формы. Брать текст из
// состояния страницы значило бы дать возможность опубликовать одно, показав
// другое.
export const aiRunAgentLabSchema = z.object({
...
export const aiAutofixLabSchema = aiVerifyLabSchema.extend({
  ...
  maxRounds: z.number().int().min(1).max(5).optional(),
  // ЧТО ИМЕННО ПРАВИТЬ. Пусто — сервер сам рецензирует кейс и правит всё, что
  // найдёт. Передан список — правится ровно он: так работает кнопка «исправить»
  // у отдельного замечания.
  //
  // Принимать замечания от клиента безопасно: гейт публикации на них не
  // стоит. После правки сервер ВСЁ РАВНО рецензирует результат сам и
  // сохраняет свою рецензию — прислав пустой или выдуманный список, можно
  // только направить правку, но не открыть публикацию.
  issues: z
    .array(
      z.object({
    ...
    .max(30)
    .optional(),
  // Указание автора редактору: чем править и в какую сторону. Главнее
  // предложения рецензента — выбор между «убрать упоминание» и «добавить
  // показатель» врачебный, а не редакторский.
  hint: z.string().trim().max(1000).optional(),
});
