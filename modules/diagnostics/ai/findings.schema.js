...
export function normalizeFindings(parsed) {
  ...
  const findings = (Array.isArray(parsed?.findings) ? parsed.findings : [])
    ...
    .filter((f) => f.title)
    // Порядок задаёт МОДЕЛЬ (правила 8 и 11 в SYSTEM_BASE): первым — ведущая
    // версия, за ней опасные версии, дальше остальное.
    //
    // Пересортировки по важности здесь больше нет. Она давала нестабильный
    // первый пункт: стоило модели пометить don't-miss версию как critical —
    // и ответ на вопрос врача уезжал с первого места на третье. Обе оценки
    // законны, но на один и тот же вопрос врач получал разный первый пункт,
    // и понять, где ответ, можно было только прочитав весь список.
    //
    // Критические выводы при этом не теряются: их число вынесено отдельной
    // строкой над списком, а по правилу 11 они идут сразу за ведущей версией.
    .slice(0, 12);
