...
const AuditLogSchema = new Schema(
  {
    ...
    /* ═══════════ TENANT ═══════════
       В какой клинике произошло действие.

       ПОЛЯ ЗДЕСЬ НЕ БЫЛО, и журнал мультиарендной системы не отвечал на
       вопрос «какая клиника обращалась к данным» — ни пациенту, ни
       комплаенс-аудиту. Восстановить это задним числом по остальным
       полям невозможно: actor может состоять в нескольких клиниках.

       null — действие вне клиники (сам пациент, платформенный сервис)
       или запись, сделанная до появления поля. Второе от первого не
       отличить, и выдавать одно за другое нельзя: в журнале для
       пациента такие строки честнее называть «клиника не указана». */
    clinicId: {
      type: Schema.Types.ObjectId,
      ref: "Clinic",
      default: null,
