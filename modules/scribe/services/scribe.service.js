...
/**
 * Граница «сеанс ещё может идти».
 *
 * Приём длиннее MAX_SESSION_SEC не бывает — дальше ingestChunk куски не
 * принимает. Значит сеанс, созданный раньше этой границы, к текущему
 * разговору отношения не имеет, кем бы он ни числился по статусу.
 *
 * Нужно потому, что комната у диалога одна на все звонки за всё время:
 * без границы вчерашний недоотвеченный сеанс находился бы снова и снова.
 */
...
export async function startSession({
  ...
  //
  // НО ТОЛЬКО ЖИВОЙ. Комната у диалога одна на все звонки за всё время
  // (dialog-<id>), поэтому без ограничения по возрасту сюда попадал
  // сеанс из ПРОШЛОГО разговора — и врач получал его назад вместо
