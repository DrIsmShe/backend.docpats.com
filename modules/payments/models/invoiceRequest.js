...
const invoiceRequestSchema = new mongoose.Schema(
  {
    ...
    // ─── Архив ────────────────────────────────────────────────────
    //
    // Отдельное поле, а не статус «archived»: статус описывает, что с
    // деньгами (новая → счёт выставлен → оплачена), а архив — что с ней
    // делает администратор. Убрать с глаз можно заявку в любом статусе,
    // и подменять одним другое значило бы терять первое.
    //
    // Между «убрать из списка» и «удалить навсегда» есть шаг намеренно:
    // заявка — это чужое намерение заплатить. Удалить её случайно значит
    // потерять клиента, который ждёт счёт и не понимает, почему тихо.
