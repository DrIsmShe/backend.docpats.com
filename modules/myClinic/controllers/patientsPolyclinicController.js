...
const getPatientsPolyclinic = async (req, res) => {
  ...
  try {
    ...
    /* ===================== 1. БАЗОВЫЕ ЗАПРОСЫ ===================== */
    const { archiveStatus } = req.query; // active | archived | all
    // Пациенты клиники (NewPatientPolyclinic)
    const clinicQuery = {
      doctorId: { $in: [doctorUserObjectId] },
      isDeleted: { $ne: true },
      isArchived: { $ne: true }, // ← по умолчанию ACTIVE
    };
    ...
    const privateQuery = {
      ...
      isArchived: { $ne: true },
      isDeleted: { $ne: true }, // ← ВОТ СЮДА
    };
    // ===================== ARCHIVE STATUS (SAFE EXTENSION) =====================
    // По умолчанию уже стоит: isArchived: { $ne: true }

    if (archiveStatus === "archived") {
      // показать ТОЛЬКО архив
      privateQuery.isArchived = true;
    } else if (archiveStatus === "all") {
      // показать и active, и archived
      delete privateQuery.isArchived;
    }
    // archiveStatus === "active" или undefined →
    // ничего не делаем, остаётся { $ne: true }
    if (archiveStatus === "archived") {
      clinicQuery.isArchived = true;
    } else if (archiveStatus === "all") {
      delete clinicQuery.isArchived;
    }
    // active / undefined → isArchived: { $ne: true }
    ...
    const clinicPatients = clinicDocs.map((d) => {
      ...
      return {
        ...
        isArchived: !!d.isArchived, // ✅ ВОТ ЭТО КЛЮЧЕВО
    ...
    const privatePatients = privateDocs.map((d) => {
      ...
      return {
        ...
        patientType: "private",
        isArchived: !!d.isArchived, // ✅ ДОБАВИТЬ
