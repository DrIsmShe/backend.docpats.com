...
const getListGastroscopyScanController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [GastroscopyScan] Запрос на список HOLTER-исследований для пациента ${id}`
  );

  try {
    const gastroscopyScansRaw = await GastroscopyScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${gastroscopyScans.length} USM-исследований`
    );
