...
const TempLaboratoryTestResultsController = async (req, res) => {
  try {
    ...
    const existingTemplate = await TempLaboratoryTestResults.findOne({ title });
    if (existingTemplate) {
      return res
        .status(400)
    ...
    const newTemplate = new TempLaboratoryTestResults({
      title,
      content,
      tags: tagsArray,
      createdBy: userId,
...
export default TempLaboratoryTestResultsController;
