...
const getListHOLTERScanController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [HOLTERScan] Запрос на список HOLTER-исследований для пациента ${id}`
  );

  try {
    const holterScansRaw = await HOLTERScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${holterScans.length} USM-исследований`
    );
