...
const getDetailExaminationsController = async (req, res) => {
  try {
    ...
    const eegScan = await EEGScan.findById(id)
    ...
    if (eegScan.doctor?.decryptFields) {
      eegScan.doctor = eegScan.doctor.decryptFields();
    }

    // Расшифровка имен врачей в комментариях
    if (eegScan.doctorComments?.length > 0) {
      eegScan.doctorComments = eegScan.doctorComments.map((comment) => {
