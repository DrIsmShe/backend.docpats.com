...
const getListXRAYScanerController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [XRAYScan] Запрос на список XRAY-исследований для пациента ${id}`
  );

  try {
    const xrayScansRaw = await XRAYScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${xrayScans.length} XRAY-исследований`
    );
    ...
    res.status(200).json({
      ...
      data: xrayScans,
      message: "Список XRAY-исследований успешно получен",
    });
  } catch (error) {
    console.error(`[${timestamp}] ❌ Ошибка получения XRAY: ${error.message}`);
    res.status(500).json({
      success: false,
