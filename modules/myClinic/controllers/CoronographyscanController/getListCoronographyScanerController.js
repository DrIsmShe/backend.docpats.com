...
const getListCoronographyScanController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [CoronographyScan] Запрос на список HOLTER-исследований для пациента ${id}`
  );

  try {
    const coronographyScansRaw = await CoronographyScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${coronographyScans.length} USM-исследований`
    );
    ...
    res.status(200).json({
      ...
      message: "Список coronography-исследований успешно получен",
    });
  } catch (error) {
