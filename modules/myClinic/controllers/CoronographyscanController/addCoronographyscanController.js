...
const addCoronographyScanController = async (req, res) => {
  try {
    ...
    const newCoronographyScan = new CoronographyScan({
      ...
      files: uploadedFiles,
      patient: patient._id,
      patientModel: patient.constructor.modelName, // 🔥 КРИТИЧНО
    ...
    try {
      await AuditLog.createLog({
        action: "CREATE_CORONOGRAPHY_SCAN",
        doctor: doctorId,
        userId: doctorId,
        patient: patient._id,
        patientModel: patientModelName,
        studyType: "CoronographyScan",
        performedOutsideSpecialization,
      ...
      console.error("❌ AuditLog error (CoronographyScan):", logErr.message);
    }
    const saved = await CoronographyScan.findById(
