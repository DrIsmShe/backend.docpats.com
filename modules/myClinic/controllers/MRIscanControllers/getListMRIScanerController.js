...
const getListMRIScanerController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [MRIScan] Запрос на список КТ-исследований для пациента ${id}`
  );

  try {
    const ctScansRaw = await MRIScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    console.log(`[${timestamp}] ✅ Получено ${ctScans.length} КТ-исследований`);

    res.status(200).json({
      success: true,
      count: ctScans.length,
      data: ctScans,
      message: "Список КТ-исследований успешно получен",
    });
  } catch (error) {
