...
const getListEchoEKGScanController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [EchoEKGScan] Запрос на список EchoEKGScan-исследований для пациента ${id}`
  );

  try {
    const echoekgScansRaw = await EchoEKGScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${echoekgScans.length} ECHO EKG-исследований`
    );
    ...
    res.status(200).json({
      ...
      data: echoekgScans,
      message: "Список ECHO EKG-исследований успешно получен",
    });
  } catch (error) {
