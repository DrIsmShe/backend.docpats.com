...
const getListSPECTScanerController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [SPECTScan] Запрос на список SPECT-исследований для пациента ${id}`
  );

  try {
    const spectScansRaw = await SPECTScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${spectScans.length} XRAY-исследований`
    );
    ...
    res.status(200).json({
      ...
      data: spectScans,
      message: "Список SPECT-исследований успешно получен",
    });
  } catch (error) {
    console.error(`[${timestamp}] ❌ Ошибка получения SPECT: ${error.message}`);
    res.status(500).json({
      success: false,
