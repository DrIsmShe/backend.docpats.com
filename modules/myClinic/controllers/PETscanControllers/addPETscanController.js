  ...
  try {
    ...
    if (!allowedSpecializations.includes(specializationName)) {
      return res.status(403).json({
        message:
          "Доступ разрешён только специалистам по радиологии, специалистам по медицинской визуализации, специалистам по функциональной диагностике.",
      });
    }
    ...
    await newPETScan.save();

    const savedPETScan = await PETScan.findById(newPETScan._id).populate(
      "files"
    );

    return res.status(201).json({
      message: "✅ PET result added successfully",
      data: savedPETScan,
    });
  } catch (error) {
