...
const getListPETScanerController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [PETScan] Запрос на список XRAY-исследований для пациента ${id}`
  );

  try {
    const petScansRaw = await PETScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    const petScans = petScansRaw.map((scan) => {
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${petScans.length} PET-исследований`
    );

    res.status(200).json({
      success: true,
      count: petScans.length,
      data: petScans,
      message: "Список PET-исследований успешно получен",
    });
  } catch (error) {
    console.error(`[${timestamp}] ❌ Ошибка получения PET: ${error.message}`);
    res.status(500).json({
      success: false,
      message: "Ошибка при получении PET-исследований",
      error: error.message,
    });
  }
};
