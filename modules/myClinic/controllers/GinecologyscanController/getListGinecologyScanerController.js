...
const getListGinecologyScanerController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [Ginecology] Запрос на список Ginecology-исследований для пациента ${id}`
  );

  try {
    const ginecologyScansRaw = await GinecologyScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    const ginecologyScan = ginecologyScansRaw.map((scan) => {
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${ginecologyScan.length} USM-исследований`
    );

    res.status(200).json({
      success: true,
      count: ginecologyScan.length,
      data: ginecologyScan,
