...
const getListDoplerScanController = async (req, res) => {
  ...
  console.log(
    `[${timestamp}] 🔍 [DoplerScan] Запрос на список HOLTER-исследований для пациента ${id}`
  );

  try {
    const doplerScansRaw = await DoplerScan.find({ patientId: id }) // Фильтруем по patientId
    ...
    console.log(
      `[${timestamp}] ✅ Получено ${doplerScans.length} USM-исследований`
    );
