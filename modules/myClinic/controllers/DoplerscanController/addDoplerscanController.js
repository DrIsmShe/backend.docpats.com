  ...
  try {
    ...
    if (!allowedSpecializations.includes(specializationName)) {
      return res.status(403).json({
        message:
          "Access is permitted only to specialists in Ultrasound Diagnostician,Medical Imaging Specialist, Radiologist,Functional Diagnostics Specialist,Cardiologist,Interventional Cardiologist,Pediatric Cardiologist,Neurosurgeon,Vascular Surgeon,Neurologist,Pediatric Neurologist.",
      });
    }
    ...
    return res.status(201).json({
      message: "✅ Результаты Dopler успешно добавлены",
      data: savedDoplerScan,
    });
  } catch (error) {
    console.error("❌ Ошибка при добавлении Dopler:", error);
