...
const getListNameofexamTemplatesAngiographyscanerController = async (
  ...
  try {
    const templates = await AngiographyScanerTemplateNameofexam.find({
      doctor: doctorId,
    })
...
// Получить список шаблонов отчёта (Report)
const getListReportTemplatesAngiographyscanerController = async (req, res) => {
  // Здесь предполагается, что ID врача хранится в req.user._id после аутентификации
  const doctorId = req.session.userId;
  try {
    const templates = await AngiographyScanerTemplateReport.find({
      doctor: doctorId,
    })
      .populate("doctor", "firstName lastName")
      .sort({ createdAt: -1 });
...
const getListDiagnosisTemplatesAngiographyscanerController = async (
  req,
  res
) => {
  // Здесь предполагается, что ID врача хранится в req.user._id после аутентификации
  const doctorId = req.session.userId;
  try {
    const templates = await AngiographycanerTemplateDiagnosis.find({
      doctor: doctorId,
    })
      .populate("doctor", "firstName lastName")
      .sort({ createdAt: -1 });
...
const getListRecomandationTemplatesAngiographyscanerController = async (
  req,
  res
) => {
  // Здесь предполагается, что ID врача хранится в req.user._id после аутентификации
  const doctorId = req.session.userId;
  try {
    const templates = await AngiographyScanerTemplateRecomandation.find({
      doctor: doctorId,
    })
      .populate("doctor", "firstName lastName")
      .sort({ createdAt: -1 });
...
export default {
  getListNameofexamTemplatesAngiographyscanerController,
  getListReportTemplatesAngiographyscanerController,
  getListDiagnosisTemplatesAngiographyscanerController,
  getListRecomandationTemplatesAngiographyscanerController,
};
