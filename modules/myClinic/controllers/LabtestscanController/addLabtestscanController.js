...
const addLabtestscanController = async (req, res) => {
  try {
    ...
    if (!isPrivatePatient) {
      if (
        !doctorSpecName ||
        !allowedLabTestSpecializations.includes(doctorSpecName)
      ) {
    ...
    if (incomingFiles.length > 0) {
      const filesToInsert = incomingFiles.map((file) => {
        ...
        return {
          ...
          patientId: patient._id, // 🔥 теперь через resolvePatient
        };
      });
    ...
    const labTestDoc = new LabTest({
      patient: patient._id,
      patientModel: patient.constructor.modelName, // 🔥 добавили
      doctor: doctorId,
    ...
    try {
      ...
      console.error("❌ AuditLog error (LabTest):", logErr.message);
    }
