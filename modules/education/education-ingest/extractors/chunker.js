...
// вопросов, заведомо под лимитом вывода.
const PDF_PAGES_PER_CHUNK = 20;

// Текст режем по 20k символов. Раньше было 40k, но плотные банки —
// короткий вопрос плюс 4–5 таких же коротких вариантов — вмещают в 40k до
// ~250 вопросов, а их разбор не влезает в лимит вывода модели. Часть тогда
// переполняется и уходит в деление пополам: результат тот же, но медленно
// и лишними вызовами. 20k держит часть заведомо под лимитом за один
// проход. Частей выходит больше, зато каждая быстрая и предсказуемая.
const TEXT_CHARS_PER_CHUNK = 20_000;
