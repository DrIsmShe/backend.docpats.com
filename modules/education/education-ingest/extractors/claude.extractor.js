...
export async function extract({
  ...
  if (message.stop_reason === "max_tokens") {
    ...
    //
    // Флаг overflow: этот кусок слишком плотный. runExtraction ловит его и
    // делит часть пополам, а не сдаётся — оператору резать файл руками
    // больше не нужно.
    throw new ValidationError(
      "Часть слишком плотная: ответ модели не поместился в лимит за один проход.",
      { overflow: true },
    );
  }
