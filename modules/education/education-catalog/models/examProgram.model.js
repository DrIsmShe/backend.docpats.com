...
const examProgramSchema = new Schema(
  {
    ...
    // Языки, на которых у программы есть вопросы. Считается автоматически по
    // банку (program.service → recountPublishedItems) и НЕ является ответом на
    // вопрос «какого языка этот тест».
    languages: {
      type: [String],
      enum: EXAM_LANGUAGES,
    ...
    // ОСНОВНОЙ язык теста — тот, к которому его относит админ.
    //
    // Зачем отдельно от languages. languages — это «на чём есть вопросы», и у
    // переведённого теста там честно лежат все пять. Врач, выбравший в
    // каталоге English, получал в выдаче «Типология личности по Карлу Юнгу» с
    // русским заголовком: тест действительно доступен на английском, но
    // отнести его к английским — значит смешать в одном списке пять
    // одинаковых карточек с нечитаемыми названиями. Каталог отвечает на
    // вопрос «что здесь есть на моём языке», а не «что я технически могу
    // открыть», и на него отвечает именно это поле.
    //
    // null = не проставлен (старые тесты) — тогда витрина откатывается к
    // languages[0], чтобы тест не исчез из каталога до того, как админ его
    // разметит.
    primaryLang: {
      type: String,
      enum: [...EXAM_LANGUAGES, null],
      default: null,
    },
