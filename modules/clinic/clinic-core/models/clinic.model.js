...
const clinicSchema = new mongoose.Schema(
  {
    ...
    //
    // Это ЯЗЫК ОРИГИНАЛА — то, что владелец ввёл первым. Переводы лежат в
    // descriptionI18n; когда для нужного языка перевода нет, показывается это
    // поле. Так старые клиники продолжают работать без миграции.
    description: { type: String, maxlength: 5000, default: "" },

    // ─────────────────────────────────────────────────────────────────────
    // ПЕРЕВОДЫ ВИТРИНЫ.
    //
    // Интерфейс платформы говорит на пяти языках, а витрина до этого умела
    // только на одном: description и slogan были обычными строками. Для рынка,
    // где «ЛОР Баку», «otorinolarinqoloq Bakı» и «ЛОР врач Баку» — три разные
    // выдачи, это означало присутствие ровно в одной из них.
    //
    // Map, а не вложенная схема: ключи — коды языков, набор которых меняется
    // без миграции, а lean() отдаёт Map обычным объектом, что и нужно DTO.
    //
    // Пустые значения НЕ хранятся (сервис их отсеивает): пустая строка в языке
    // и отсутствие языка — одно и то же, а различать их значит городить
    // фолбэк на пустоту.
    // ─────────────────────────────────────────────────────────────────────
    descriptionI18n: { type: Map, of: String, default: undefined },
    sloganI18n: { type: Map, of: String, default: undefined },
    // Язык оригинала — на него ссылается hreflang x-default и он же
    // подставляется, когда перевода нет.
    originalLanguage: {
      type: String,
      enum: ["ru", "en", "az", "tr", "ar"],
      default: "ru",
    ...
    // ─── Пробный период клиники ───────────────────────────────────
    //
    // Бесплатного клинического тарифа нет и не будет: клиника — это
    // организация, у неё есть бюджет, и продукт для неё стоит денег.
    // Но начать работу, не заплатив вперёд, она должна — иначе решение
    // о покупке принимается вслепую.
    //
    // Поэтому здесь срок, а не тариф. Пока он идёт, клиника работает на
    // лимитах Start. Когда он вышел и оплаты нет — клиника ЗАМОРАЖИВАЕТСЯ:
    // чтение остаётся, запись закрывается (см. clinicWriteGate).
    //
    // Чтение не отключаем сознательно. В клинике лежат медицинские
    // карты пациентов, и закрывать к ним доступ из-за неоплаченного
    // счёта — не коммерческое давление, а угроза для людей, которые к
    // счёту отношения не имеют. Коммерческая цель достигается запретом
    // на НОВЫЕ записи: работать нельзя, а прошлое остаётся доступным.
