...
const SimulationSchema = new Schema(
  {
    ...
    // Насколько результат отличается от исходного снимка (среднее
    // отличие яркости, 0-255) и признак того, что правки практически нет.
    // См. changeScore.js: модели редактирования недетерминированы и время
    // от времени возвращают кадр нетронутым.
    changeScore: { type: Number, default: null },
    weakChange: { type: Boolean, default: false },
    // Почему перевод не состоялся: "HTTP 429", "таймаут", "нет
    // OPENAI_API_KEY". Раньше отказ компилятора был невидим — в модель
    // молча уходил русский текст врача, она его не понимала и возвращала
    // почти исходный кадр. Снаружи это выглядело как «ИИ ничего не сделал»,
    // и отличить это от неудачной правки было нечем.
    promptCompileReason: { type: String, default: null },
