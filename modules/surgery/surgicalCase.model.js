...
const PhotoRefSchema = new Schema(
  {
    ...
    // Ракурс съёмки. Отдельно от label, потому что это разные вопросы:
    // label отвечает «когда снято» (до операции, после, через месяц),
    // view — «с какой стороны».
    //
    // Нужен он симуляции. Горбинка носа видна только в профиль, и запрос
    // «убрать горбинку», применённый к анфасу, модель честно отрабатывает
    // ничем: убирать на этом снимке нечего. Раньше все снимки грузились с
    // одной меткой "before" и подписывались одинаковым «До» — врач не
    // видел, какой из трёх кадров профильный, брал первый и получал
    // «ИИ ничего не сделал».
    //
    // Пустая строка — «ракурс не указан»: у снимков, загруженных до
    // появления поля, его знать неоткуда, и выдумывать за врача нельзя.
    view: {
      type: String,
      enum: ["", "front", "profile", "three_quarter", "other"],
      default: "",
    },
...
const SurgicalCaseSchema = new Schema(
  {
    ...
    // ─── Международный код вмешательства ────────────────────────────────────
    //
    // ОТДЕЛЬНОЕ поле, а не расширение enum `procedure` выше. Это разные вещи:
    //   procedure        — наша внутренняя классификация ("rhinoplasty"),
    //                      по ней строятся подборки, права и интерфейс;
    //   interventionCode — международная номенклатура (ICD-9-CM Vol.3, дальше
    //                      ICHI), она уходит в отчётность и страховые расчёты.
    //
    // Смешать их в одном поле означало бы либо потерять внутреннюю группировку,
    // либо привязать отчётность к нашим строкам, которых нет ни в одном
    // классификаторе. Поле необязательное: код проставляется, когда он нужен,
    // и заполняет его человек — подбор кода машиной остаётся подсказкой.
    //
    // Форма повторяет mainDiagnosis в медкарте (code + codeTitle + system),
    // чтобы врач видел одинаковую структуру в диагнозе и в операции.
    interventionCode: {
      // Система кодирования: значение из CODE_SYSTEMS модуля medicalCodes.
      // Хранится рядом с кодом, потому что "28.4" в разных системах — разные
      // вмешательства, и без системы код нечитаем.
      system: { type: String, trim: true, default: "" },
      code: { type: String, trim: true, default: "" },
      // Официальное название кода на момент выбора. Дублирует справочник
      // намеренно: номенклатуры пересматриваются, и запись об операции должна
      // сохранить формулировку, которую видел врач, когда её подписывал.
