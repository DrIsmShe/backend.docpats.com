...
export async function createSimulation(
  ...
  const simulation = await Simulation.create({
    ...
    // и четыре варианта превращали симуляцию в $0.66 — дороже всего
    // остального на платформе вместе взятого.
    //
    // По умолчанию один. Четыре варианта имели смысл, пока модель рисовала
    // непредсказуемо и из них приходилось выбирать удачный; gpt-image-2
    // правит ровно то, о чём просят, и разброс между вариантами свёлся к
    // мелочам текстуры. Врач всё равно выбирал один.
    //
    // Поднимается без правки кода: SIMULATION_VARIANTS=2..4 — но помните,
    // что квоты тарифов посчитаны из расчёта одного варианта.
    numOutputs: Math.min(4, Math.max(1, Number(process.env.SIMULATION_VARIANTS) || 1)),
    disclaimerAccepted,
    status: "pending",
  });
