...
export async function рекомендации({ viewer = null, limit = 24, lang = "" } = {}) {
  ...
  const подобранные = кандидаты
    .map((v) => ({ v, балл: оценить(v, п, сейчас) }))
    .sort((a, b) => b.балл - a.балл)
    .slice(0, limit)
    .map((x) => x.v);

  // ЛЕНТА НЕ ДОЛЖНА СЖИМАТЬСЯ ОТ ТОГО, ЧТО ЧЕЛОВЕК СМОТРИТ.
  //
  // Просмотренное исключается, чтобы подборка не повторял вчерашнее —
  // но на молодом каталоге это значит, что посмотревший четыре ролика из
  // семи видит на главной три. Выглядит как потерянные ролики, а не
  // как забота. Поэтому недобор дополняем уже виденным — в конец, где
  // оно не мешает новому.
  if (подобранные.length >= limit || !исключить.length) return подобранные;

  const взято = new Set(подобранные.map((v) => String(v._id)));
  const добор = await Video.find({
    ...базовый,
    _id: { $in: исключить },
  })
    .sort({ publishedAt: -1 })
    .limit(limit - подобранные.length)
    .select(поля)
    .lean();
