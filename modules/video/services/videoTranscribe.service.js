...
//
// ПРЕДЕЛ 25 МБ — ЧУЖОЙ, И ОБОЙТИ ЕГО НЕЧЕМ. Whisper принимает файл до 25 МБ;
// вытащить из видео одну звуковую дорожку мы не можем — ffmpeg на сервере
// нет. Поэтому крупные ролики получают честный отказ с объяснением, а не
// молчаливую неудачу. Когда ffmpeg появится, менять придётся один этот
// файл: всё остальное работает с текстом.
//
...
export async function transcribeVideo({ actor, id, targets = [], lang = "" }) {
  ...
  const размер = video.media.sizeBytes || 0;
  if (размер > ПРЕДЕЛ_БАЙТ) {
    throw new ValidationError(
      `Распознавание принимает файлы до ${Math.floor(ПРЕДЕЛ_БАЙТ / 1024 / 1024)} МБ, ` +
        `а в ролике ${Math.round(размер / 1024 / 1024)} МБ. ` +
        "Пересоберите ролик компактнее или загрузите его частями.",
    );
  }
