  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      ...
      ({
        ...
        const callerUserId = userId || clientUserId;

        console.log(`📞 [OFFER] от ${socket.id} (${callerRole})`);

        // сохраняем информацию о том, кто начал звонок
    ...
    socket.on("call:end", async (roomId, meta = {}) => {
      ...
      const callData = {
        callSessionId: callSessionId || `${roomId}-${Date.now()}`,
        roomId,
        ...
        callee:
          initiator && initiator.role !== currentRole ? currentRole : "unknown",
        callerUserId: initiator?.userId || userId,
        calleeUserId: userId,
        callerName: initiator?.name || userName,
