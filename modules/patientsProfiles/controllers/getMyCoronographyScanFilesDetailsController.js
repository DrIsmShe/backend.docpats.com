// server/modules/patientProfile/controllers/getMyCoronographyScanFilesDetailsController.js
...
/**
 * GET /patient-profile/get-my-coronography-file-details/files/:id
 * Возвращает детальную информацию о CoronographyScan вместе с файлами/медиа.
 *
 * Порядок поиска:
 *   1) по _id (req.params.id)
 *   2) резерв: по files.studyReference == :id
 */
export default async function getMyCoronographyScanFilesDetailsController(
  req,
  res
) {
  try {
    ...
    if (!anyId) {
      return res
        .status(400)
        .json({ ok: false, error: "INVALID_CORONOGRAPHY_ID" });
    }

    // Базовый запрос с populate ключевых связей
    let q = CoronographyScan.findById(anyId)
      .populate({
        path: "doctor",
        select:
    ...
    if (!doc) {
      doc = await CoronographyScan.findOne({ "files.studyReference": anyId })
        .populate({
          path: "doctor",
          select:
            "role username email firstName lastName firstNameEncrypted lastNameEncrypted",
    ...
    const item = {
      ...
      radiationDose:
        typeof doc.radiationDose === "number" ? doc.radiationDose : null,
      ...
      predictionConfidence:
        typeof doc.predictionConfidence === "number"
          ? doc.predictionConfidence
          : null,
      aiProcessingTime:
        typeof doc.aiProcessingTime === "number" ? doc.aiProcessingTime : null,
    ...
    console.error("[getMyCoronographyScanFilesDetailsController] error:", err);
