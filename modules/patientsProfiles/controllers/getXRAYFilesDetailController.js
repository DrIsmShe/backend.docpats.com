// server/modules/xray/controllers/getXRAYFilesDetailController.js
import mongoose from "mongoose";
// ⬇️ Укажите корректный путь к вашей модели XRayScan
import XRayScan from "../../../common/models/Polyclinic/ExamenationsTemplates/XRayScansTemplates/XRayScan.js";

/* ========== helpers (как в CT/USM) ========== */
const getUserId = (req) =>
  req.userId ||
  req.user?._id ||
  req.session?.userId ||
...
/** Если у User/Patient есть decryptFields — берём расшифрованные имена */
...
/* ========== controller ========== */
// GET /patient-profile/get-my-xray-file-details/files/:id
export default async function getXRAYFilesDetailController(req, res) {
  try {
    // ---- авторизация
    ...
    if (!anyId) {
      return res.status(400).json({ ok: false, error: "INVALID_XRAY_ID" });
    }
    ...
    const POPULATE_SPEC = safePopulateSpec(XRayScan.schema, CANDIDATE_POPULATE);

    // ---- 1) поиск по _id
    let q = XRayScan.findById(anyId).setOptions({ strictPopulate: false });
    for (const p of POPULATE_SPEC) q = q.populate(p);
    let doc = await q.exec();

    // ---- 2) резерв: поиск по files.studyReference
    if (!doc) {
      let q2 = XRayScan.findOne({
    ...
    // ---- ответ (единый формат как CT/USM)
    const item = {
      _id: String(doc._id),
      date: doc.date,
    ...
    console.error("[getXRAYFilesDetailController] error:", err);
