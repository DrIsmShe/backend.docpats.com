// server/modules/patientProfile/controllers/getMyDoplerFilesDetailsController.js
...
/**
 * GET /.../dopler/files/:id
 * Возвращает детальную информацию об исследовании Доплера с файлами.
 *
 * Порядок поиска:
 *   1) по _id (req.params.id)
 *   2) резерв: по files.studyReference == :id
 */
export default async function getMyDoplerFilesDetailsController(req, res) {
  try {
    const idParam = String(req.params?.id || "").trim();
    const anyId = toObjectId(idParam);
    if (!anyId) {
      return res.status(400).json({ ok: false, error: "INVALID_DOPLER_ID" });
    }

    // Базовый запрос + populate ключевых связей
    let q = DoplerScan.findById(anyId)
      .populate({
        path: "doctor",
        select:
      ...
      // В вашей схеме ref называется DoplerScannTemplateReport (с двойной n) — оставляем как есть:
    ...
    if (!doc) {
      doc = await DoplerScan.findOne({ "files.studyReference": anyId })
        .populate({
          path: "doctor",
          select:
            "role username email firstName lastName firstNameEncrypted lastNameEncrypted",
    ...
    console.error("[getMyDoplerFilesDetailsController] error:", err);
