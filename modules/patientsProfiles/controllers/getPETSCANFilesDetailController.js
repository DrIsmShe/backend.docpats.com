// server/modules/petScan/controllers/getPETSCANFilesDetailController.js
import mongoose from "mongoose";
// ⬇️ ВАЖНО: проверьте путь к модели и регистр букв!
import PETScan from "../../../common/models/Polyclinic/ExamenationsTemplates/PETScansTemplates/PETScan.js";

/* ========== helpers (как в CT/USM/XRAY) ========== */
const getUserId = (req) =>
  req.userId ||
  req.user?._id ||
  req.session?.userId ||
...
/* ========== controller ========== */
// GET /patient-profile/get-my-pet-scan-file-details/files/:id
export default async function getPETSCANFilesDetailController(req, res) {
  try {
    ...
    const POPULATE_SPEC = safePopulateSpec(PETScan.schema, CANDIDATE_POPULATE);

    // ---- 1) поиск по _id
    let q = PETScan.findById(anyId).setOptions({ strictPopulate: false });
    for (const p of POPULATE_SPEC) q = q.populate(p);
    let doc = await q.exec();

    // ---- 2) резерв: поиск по files.studyReference
    if (!doc) {
      let q2 = PETScan.findOne({
    ...
    const item = {
      ...
      // Специфика PET (радиация/область исследования)
    ...
    console.error("[getPETSCANFilesDetailController] error:", err);
