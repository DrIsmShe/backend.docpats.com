// server/modules/patientProfile/controllers/getMyEKGScanFilesDetailsController.js
...
/**
 * GET /.../ekg/files/:id
 * Возвращает детальную информацию об ЭКГ (EKGScans) вместе с файлами/медиа.
 *
 * Порядок поиска:
 *   1) по _id (req.params.id)
 *   2) резерв: по files.studyReference == :id
 */
export default async function getMyEKGScanFilesDetailsController(req, res) {
  try {
    const idParam = String(req.params?.id || "").trim();
    const anyId = toObjectId(idParam);
    if (!anyId) {
      return res.status(400).json({ ok: false, error: "INVALID_EKG_ID" });
    }

    // Базовый запрос с populate ключевых связей
    let q = EKGScans.findById(anyId)
      .populate({
        path: "doctor",
        select:
    ...
    if (!doc) {
      doc = await EKGScans.findOne({ "files.studyReference": anyId })
        .populate({
          path: "doctor",
          select:
            "role username email firstName lastName firstNameEncrypted lastNameEncrypted",
    ...
    const item = {
      ...
      // Поля из схемы (оставляем как есть, если используются)
    ...
    console.error("[getMyEKGScanFilesDetailsController] error:", err);
