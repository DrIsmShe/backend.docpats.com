...
//
// ========== helpers (идентично CT-контроллеру) ==========
//
const getUserId = (req) =>
  req.userId ||
  req.user?._id ||
...
//
// ========== controller ==========
// GET /patient-profile/get-my-usm-file-details/files/:id
//
export default async function getUSMFilesDetailController(req, res) {
  try {
    ...
    if (!anyId) {
      return res.status(400).json({ ok: false, error: "INVALID_USM_ID" });
    }

    // Подготовим спецификацию populate безопасно по схеме
    ...
    const POPULATE_SPEC = safePopulateSpec(USMScans.schema, CANDIDATE_POPULATE);

    // 1) пробуем по _id
    let q = USMScans.findById(anyId).setOptions({ strictPopulate: false });
    for (const p of POPULATE_SPEC) q = q.populate(p);
    let doc = await q.exec();

    // 2) резерв: поиск по files.studyReference
    if (!doc) {
      let q2 = USMScans.findOne({ "files.studyReference": anyId }).setOptions({
    ...
    /* ===== Формируем ответ в том же формате, что и CT-контроллер ===== */
    const item = {
      _id: String(doc._id),
      date: doc.date,
      ...
      // Специфика УЗИ
      radiationDose: doc.radiationDose ?? null, // обычно неактуально, но поле есть в схеме
      contrastUsed: !!doc.contrastUsed,
      dopplerFindings: doc.dopplerFindings || "",
      echogenicity: doc.echogenicity || "",
      probeFrequency:
    ...
    console.error("[getUSMFilesDetailController] error:", err);
