// server/modules/patientProfile/controllers/getMySpirometryFilesDetailsController.js
...
/**
 * GET /.../spirometry/files/:id
 * Возвращает детальную информацию о спирометрии с файлами.
 *
 * Порядок поиска:
 *  1) по _id (req.params.id)
 *  2) резерв: по files.studyReference == :id
 */
export default async function getMySpirometryFilesDetailsController(req, res) {
  try {
    ...
    if (!anyId) {
      return res
        .status(400)
        .json({ ok: false, error: "INVALID_SPIROMETRY_ID" });
    }

    // Базовый запрос с populate основных связей
    let q = SpirometryScan.findById(anyId)
      .populate({
        path: "doctor",
        select:
    ...
    if (!doc) {
      doc = await SpirometryScan.findOne({ "files.studyReference": anyId })
        .populate({
          path: "doctor",
          select:
            "role username email firstName lastName firstNameEncrypted lastNameEncrypted",
    ...
    const item = {
      ...
      fvc: doc.fvc ?? null,
      fev1: doc.fev1 ?? null,
      fev1FvcRatio: doc.fev1FvcRatio ?? null,
      pef: doc.pef ?? null,

      // Технические/служебные (оставлены на случай совместимости модели)
    ...
    console.error("[getMySpirometryFilesDetailsController] error:", err);
