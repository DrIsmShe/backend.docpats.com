...
const changePasswordProfileOfPatientController = async (req, res) => {
  try {
    ...
    if (!currentPassword || !newPassword || !renewPassword) {
      return res.status(400).json({ message: tReq(req, "app.validation.allFieldsRequired") });
    }

    if (newPassword !== renewPassword) {
      return res
        .status(400)
        .json({ message: tReq(req, "app.password.confirmationMismatch") });
    }

    if (newPassword.length < 8) {
      return res
        .status(400)
        .json({ message: tReq(req, "app.password.minLength") });
    }
    ...
    if (!isCurrentPasswordValid) {
      return res.status(400).json({ message: tReq(req, "app.password.currentIncorrect") });
    }

    if (await argon2.verify(existingUser.password, newPassword)) {
      return res
        .status(400)
        .json({ message: tReq(req, "app.password.mustBeDifferent") });
    }
    ...
    return res.status(200).json({ message: tReq(req, "app.password.changeSuccess") });
  } catch (error) {
