...
/**
 * GET /.../echoekg/files/:id
 * Возвращает детальную информацию об EchoEKGScan вместе с файлами/медиа.
 *
 * Порядок поиска:
 *   1) по _id (req.params.id)
 *   2) резерв: по files.studyReference == :id
 */
export default async function getMyECHOEKGScanFilesDetailsController(req, res) {
  try {
    const idParam = String(req.params?.id || "").trim();
    const anyId = toObjectId(idParam);
    if (!anyId) {
      return res.status(400).json({ ok: false, error: "INVALID_ECHOEKG_ID" });
    }

    // Базовый запрос с populate ключевых связей
    let q = EchoEKGScan.findById(anyId)
      .populate({
        path: "doctor",
        select:
    ...
    if (!doc) {
      doc = await EchoEKGScan.findOne({ "files.studyReference": anyId })
        .populate({
          path: "doctor",
          select:
            "role username email firstName lastName firstNameEncrypted lastNameEncrypted",
    ...
    console.error("[getMyECHOEKGScanFilesDetailsController] error:", err);
