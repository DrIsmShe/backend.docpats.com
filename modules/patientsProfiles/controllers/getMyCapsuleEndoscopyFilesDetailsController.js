// server/modules/patientProfile/controllers/getMyCapsuleEndoscopyFilesDetailsController.js
import mongoose from "mongoose";
// ⚠️ ПРОВЕРЬТЕ ПУТЬ К МОДЕЛИ (ниже — примерная локация):
...
/**
 * GET /.../capsule-endoscopy/files/:id
 * Возвращает детальную информацию о Capsule Endoscopy с файлами.
 *
 * Порядок поиска:
...
export default async function getMyCapsuleEndoscopyFilesDetailsController(
  ...
  try {
    ...
    if (!anyId) {
      return res
        .status(400)
        .json({ ok: false, error: "INVALID_CAPSULE_ENDOSCOPY_ID" });
    }

    // Базовый запрос + populate ключевых связей
    let q = CapsuleEndoscopyScan.findById(anyId)
      .populate({
        path: "doctor",
        select:
      ...
      // В схеме указано 'CapsuleEndoscopyScannTemplateReport' (двойная n) — учитываем это:
    ...
    const item = {
      ...
      // Специфика капсульной эндоскопии / колоно
    ...
    console.error("[getMyCapsuleEndoscopyFilesDetailsController] error:", err);
