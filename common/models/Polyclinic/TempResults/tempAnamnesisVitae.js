...
// Define schema for Anamnesis Vitae template
const tempAnamnesisVitae = new mongoose.Schema(
  {
...
tempAnamnesisVitae.pre("save", function (next) {
  this.updatedAt = Date.now(); // Ensure updatedAt reflects the current time
  next();
});

// Create a method to search by title
tempAnamnesisVitae.methods.findByTitle = function (title) {
  return this.model("TempAnamnesisVitae").find({
    title: new RegExp(title, "i"), // Case-insensitive search
  });
};

// Create and export the model
const TempAnamnesisVitae = mongoose.model(
  "TempAnamnesisVitae",
  tempAnamnesisVitae
);

export default TempAnamnesisVitae;
