...
// Определяем схему для шаблона рекомендаций
const tempRecommendations = new mongoose.Schema(
  {
    // Название шаблона рекомендации
    title: {
    ...
    // Содержимое шаблона рекомендации
    content: {
      type: String, // Тип данных строка
      required: true, // Поле обязательно для заполнения
...
tempRecommendations.pre("save", function (next) {
...
tempRecommendations.methods.findByTitle = function (title) {
  return this.model("TempRecommendations").find({
...
tempRecommendations.statics.findByTags = function (tags) {
  return this.find({
    tags: { $in: tags }, // Ищем шаблоны, в которых есть хотя бы один из указанных тегов
  });
};

// Создаем и экспортируем модель шаблона рекомендаций
const TempRecommendations = mongoose.model(
  "TempRecommendations",
  tempRecommendations
);
