...
const tempAnamnesisMorbi = new mongoose.Schema(
  {
...
tempAnamnesisMorbi.pre("save", function (next) {
  this.updatedAt = Date.now(); // Ensure updatedAt reflects the current time
  next();
});

// Create a method to search by title
tempAnamnesisMorbi.methods.findByTitle = function (title) {
  return this.model("TempAnamnesisMorbi").find({
    title: new RegExp(title, "i"), // Case-insensitive search
  });
};

// Create and export the model
const TempAnamnesisMorbi = mongoose.model(
  "TempAnamnesisMorbi",
  tempAnamnesisMorbi
);

export default TempAnamnesisMorbi;
