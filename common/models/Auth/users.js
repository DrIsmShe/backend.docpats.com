...
const userSchema = new mongoose.Schema(
  {
    ...
    */
    ...
    // ─── Подборка предстоящих конференций (еженедельно, врачам) ───
    // Опт-аут, а не опт-ин: рубрика бесполезна, если о ней никто не узнал.
    // Отписка — ссылкой из самого письма, без входа в аккаунт.
    conferenceDigestEnabled: { type: Boolean, default: true },
    lastConferenceEmailAt: { type: Date, default: null },
    // Какие из 14 категорий врач хочет видеть. ПУСТОЙ МАССИВ = все, и это
    // не заглушка: пограничные темы — норма, а не исключение. Кардиоонколог
    // читает и кардиологию, и онкологию; конференция по ИИ в медицине не
    // относится ни к одной специальности и нужна всем. Угадывать за врача
    // мы не беремся — сузить список он может сам, в настройках.
    conferenceCategories: { type: [String], default: [] },
    ...
    /* ───── Публикация учебной активности ─────
       Врач сам решает, показывать ли на профиле, сколько учебных
       снимков он разобрал и с какой точностью.

       ПО УМОЛЧАНИЮ ВЫКЛЮЧЕНО, и это не осторожность. Автоматическая
       публикация точности наказывает того, кто тренируется и
       ошибается, — то есть ровно того, ради кого тренажёр сделан.
       Врач, увидевший невысокий процент на публичном профиле,
       перестанет тренироваться, и платформа потеряет и данные, и
       смысл модуля. */
    publicCompetence: {
    ...
    // Язык ЧЕЛОВЕКА: интерфейс, уведомления, письма. Меняется осознанно, в
    // профиле, и больше ниоткуда.
    preferredLanguage: {
      type: String,
      enum: ["en", "ru", "az", "tr", "ar"],
      default: "en",
    ...
    // Язык, на который переводить входящие сообщения в чате. ОТДЕЛЬНОЕ
    // поле, и это не педантизм: раньше выбор в чате писался прямо в
    // preferredLanguage, и врач, один раз переключившийся на арабский, чтобы
    // прочитать сообщение пациента, навсегда получал арабские напоминания о
    // приёмах. Здесь нет enum: переводить можно на любой ISO-код, а
    // интерфейс существует только на пяти языках.
    chatTranslationLang: { type: String, default: null },
