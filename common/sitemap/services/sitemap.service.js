...
/*
 * В КАРТУ ИДЁТ НЕ ВЕСЬ ДАЙДЖЕСТ, А ТОЛЬКО КРЕПКАЯ ЧАСТЬ.
 *
 * Права на текст — уже не вопрос: изложение наше. Вопрос в другом, и он
 * про качество. Правило Google о массовом производстве страниц (scaled
 * content abuse, март 2024) смотрит не на то, кто владеет текстом, а на
 * то, не сделаны ли тысячи страниц ради выдачи. Шесть с половиной тысяч
 * материалов на пяти языках — это 33 тысячи адресов по три-четыре
 * предложения каждый, и такая форма подпадает под правило независимо от
 * авторства. Санкция по нему накладывается на ДОМЕН — под ударом
 * оказались бы витрины клиник и профили врачей.
 *
 * Поэтому отбор: в карту идут материалы, у которых правила распознали
 * дизайн исследования — метаанализ, РКИ, когорта (evidenceLevel high или
 * moderate). Это 1730 материалов из 6634: настоящие исследования, по
 * которым врач принимает решения, и наше изложение действительно
 * экономит ему время. Остальное — заметки, колонки и работы, где дизайн
 * не распознан, — остаётся в продукте, но поисковику не предлагается.
 *
 * Планка меняется одной строкой ниже. Поднять её до одного only-"high"
 * (593 материала) или опустить, добавив "low", — вопрос того, сколько
 * страниц домен готов отвечать за качество.
 */
const УРОВНИ_В_КАРТЕ = ["high", "moderate"];
      ...
      .find(
        {
          ...
          evidenceLevel: { $in: УРОВНИ_В_КАРТЕ },
        },
