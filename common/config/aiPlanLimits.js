...
export const PLAN_LIMITS = {
  ...
  //
  // ЗАЧЕМ ОН ПОЯВИЛСЯ. До сих пор такого уровня не было вовсе, и откат
  // вёл на doctor_lite — то есть на ПЛАТНЫЙ тариф за 9 $. Врач, который
  // перестал платить, продолжал получать 500 вопросов, 5 разборов,
  // 30 пациентов и час видео бессрочно и даром. Неоплата ничего не
  // меняла, и «блокировка при неоплате» существовала только на словах.
  //
  // НУЛЕЙ ЗДЕСЬ БЫТЬ НЕ МОЖЕТ. Все модули трактуют 0 как «фича у плана не
  // описана» и в этом случае предел НЕ применяют (см. videoQuota,
  // storageQuota, diagnostics/quota, consultation). Ноль означал бы не
  // «нельзя», а «сколько угодно» — ровно наоборот. Поэтому минимум 1.
  //
  // Смысл уровня: аккаунт остаётся живым и данные доступны, но вести на
  // нём практику нельзя. Уже заведённые пациенты и загруженные файлы не
  // пропадают — блокируется только добавление нового.
  doctor_free: {
    examQuestions: 250, // как у patient_free: банк вопросов готов, обращений к модели нет
    aiAnalyses: 1,
    aiArticles: 1,
    soapEpicrises: 1,
