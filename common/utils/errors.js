...
export function toErrorResponse(err) {
  ...
  // ИСЧЕРПАН ЛИМИТ КОЛЛЕКЦИЙ КЛАСТЕРА.
  //
  // Общие кластеры Atlas ограничены 500 коллекциями на все базы сразу.
  // Когда предел выбран, любая первая запись в новую коллекцию падает,
  // и без этой ветки человек видел безликое «Не удалось выполнить
  // действие» — то есть искал ошибку в своём вводе там, где её нет.
  // Сообщаем прямо: это не его действие неверно, а хранилище упёрлось
  // в потолок, и чинится это не повтором кнопки.
  if (/already using \d+ collections/i.test(err?.message || "")) {
    return {
      status: 507,
      body: {
        error:
          "Хранилище исчерпало предел коллекций кластера — новые сущности создавать нельзя. Освободите место в базе и повторите.",
        code: "STORAGE_COLLECTION_LIMIT",
      },
    };
  }
