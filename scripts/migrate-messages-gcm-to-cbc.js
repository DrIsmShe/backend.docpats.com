// server/scripts/migrate-messages-gcm-to-cbc.js
//
// One-time migration: messages.textEncrypted from
// GCM+SURGERY_ENCRYPTION_KEY to CBC+ENCRYPTION_KEY.
//
// Same pattern as previous migrations. Idempotent.
...
async function migrate() {
  ...
  const coll = mongoose.connection.db.collection("messages");

  const total = await coll.countDocuments({});
  console.log(`[migrate] Total messages: ${total}`);
  ...
  for await (const doc of cursor) {
    if (!isOldGcmFormat(doc.textEncrypted)) {
      skipped++;
      continue;
    }
    try {
      const plain = decryptOldGcm(doc.textEncrypted);
      if (plain === null) {
        throw new Error("Failed to decrypt textEncrypted with OLD key");
      }
      const newEncrypted = encryptNewCbc(plain);
      await coll.updateOne(
        { _id: doc._id },
        { $set: { textEncrypted: newEncrypted } },
      );
      migrated++;
    } catch (err) {
      failed++;
