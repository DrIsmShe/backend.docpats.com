...
import registerCallHandlers from "./modules/communication/socketHandlers.js";
...
const corsOptions = {
  ...
  credentials: true, // ✅ это обязательно, иначе кука не передаётся
...
app.get("/", (req, res) => {
  res.send("✅ Backend Docpats API is running");
});
...
async function bootstrap(startPort = PORT) {
  try {
    ...
    const { default: autoCleanAppointments } = await import(
    ...
    httpServer.on("error", (err) => {
      if (err.code === "EADDRINUSE") {
        ...
        console.warn(`⚠️ Порт ${startPort} занят. Пробуем ${next}...`);
        bootstrap(next);
      } else throw err;
    });
  } catch (err) {
